...
class Dataset(object):
    ...
    def __next__(self):
        with tf.device('/cpu:0'):
            ...
            self.train_output_sizes = self.train_input_size // self.strides  # [26,13]
            ...
            exceptions = False
            num = 0
            if self.batch_count < self.num_batchs:  #0<  22(num imges\ batchsize=4)
                while num < self.batch_size:  #0,1,2...4
                    ...
                    batch_image[num, :, :, :] = image   # 4x416x416x3
                    batch_label_mbbox[num, :, :, :, :] = label_mbbox   # 4x26x26x3x(5+c)
                    batch_label_lbbox[num, :, :, :, :] = label_lbbox   # 4x13x13x3x(5+c)
                    batch_mbboxes[num, :, :] = mbboxes   # 4x100x4 in which only first few entries are filled, no of bounding box in 1 image
    ...
    def preprocess_true_boxes(self, bboxes):
        ...
        label = [np.zeros((self.train_output_sizes[i], self.train_output_sizes[i], self.anchor_per_scale,
                           5 + self.num_classes)) for i in range(OUTPUT_LEVELS)] # 13*13*3* 5+class,   26*26*3* 5+class
        ...
        for bbox in bboxes:
          ...
          #  print(smooth_onehot)
            ...
            for i in range(OUTPUT_LEVELS):#range(3):
                anchors_xywh = np.zeros((self.anchor_per_scale, 4))  #3x4
                anchors_xywh[:, 0:2] = np.floor(bbox_xywh_scaled[i, 0:2]).astype(np.int32) + 0.5
                anchors_xywh[:, 2:4] = self.anchors[i]

                iou_scale = bbox_iou(bbox_xywh_scaled[i][np.newaxis, :], anchors_xywh)  # 1 --> 1x4   2--> 3x4

                iou.append(iou_scale)
                iou_mask = iou_scale > 0.3   # [True, false, false] like result for each anchore box
        ...
        if self.train_yolo_tiny:
            ...
            mbboxes, lbboxes = bboxes_xywh  # 100 x 4 but only amount of bounding box in image will be filled, other once will be zero
